...
# If assert_np_arrays_equal_after_sorting is defined in a helper, ensure it's imported.
# For now, assuming it's defined within this file or globally available.
...
class TestSpatialGridIndex(unittest.TestCase):
    def setUp(self):
        # Ensure a clean Taichi state for each test
        try:
            # Reset Taichi runtime to clear any state from previous tests
            ti.reset()
            # Re-initialize Taichi. It's crucial to match the initialization parameters
            # used by the main code (spatial_grid_index.py).
            ti.init(
    ...
    def test_empty_input(self):
        ...
        self.assertTrue(
            index.grid_dim_x == 0 or index.grid_dim_x == 1
        )  # Can be 1 if min_x=max_x leads to resolution-sized grid
        ...
        self.assertEqual(
            empty_cell_query.size,
            0,
            "Cell query on empty index should return empty array.",
        )

        empty_radius_query = index.query_points_in_radius(0.0, 0.0, 1.0)
        self.assertEqual(
            empty_radius_query.size,
            0,
            "Radius query on empty index should return empty array.",
        )
    ...
    def test_indexed_points_length(self):
        points = np.array(
            [[0.1, 0.1], [0.2, 0.2], [2.0, 2.0], [2.5, 2.5]], dtype=np.float32
        )
        ...
        self.assertEqual(
            index_auto_extent.indexed_point_indices.shape[0],
            points.shape[0],
        ...
        custom_extent = (
            ...
            2.0,
            2.0,
        )  # max_x, max_y are exclusive for cell calculation effectively
        index_custom_extent = SpatialGridIndex(
            points_inside, resolution=1.0, extent=custom_extent
        )
        # Points (0.5,0.5) -> cell (0,0). Point (1.5,1.5) -> cell (1,1)
        # All points should be counted.
        self.assertEqual(
            index_custom_extent.indexed_point_indices.shape[0], points_inside.shape[0]
        )
        ...
        points_mixed = np.array(
            [[0.5, 0.5], [3.5, 3.5]], dtype=np.float32
        )  # Second point outside extent (0,0,2,2)
        index_mixed_extent = SpatialGridIndex(
            points_mixed, resolution=1.0, extent=custom_extent
        )
        ...
        self.assertEqual(
            index_mixed_extent.indexed_point_indices.shape[0],
            1,
    ...
    def test_qpc_empty_cell(self):
        ...
        index = SpatialGridIndex(
            points, resolution=1.0, extent=(0, 0, 2, 2)
        )  # point [0] is in cell (0,0)
    ...
    def test_qpc_multiple_points_cell(self):
        points = np.array(
            [[0.5, 0.5], [0.6, 0.6], [1.5, 0.5]], dtype=np.float32
        )  # First two in cell (0,0)
    ...
    def test_qpc_out_of_bounds(self):
        points = np.array([[0.5, 0.5]], dtype=np.float32)
        index = SpatialGridIndex(
            points, resolution=1.0, extent=(0, 0, 1, 1)
        )  # grid_dim is (1,1), valid cell (0,0)
    ...
    def test_qpr_find_multiple_points(self):
        ...
        index = SpatialGridIndex(points, resolution=1.0, extent=(-1, -1, 6, 6))
        indices = index.query_points_in_radius(
            0.0, 0.0, 0.5
    ...
    def test_qpr_radius_covers_multiple_cells(self):
        points = np.array(
            [[0.5, 0.5], [1.5, 1.5]], dtype=np.float32
        )  # Pt [0] in cell (0,0), Pt [1] in cell (1,1)
        index = SpatialGridIndex(points, resolution=1.0, extent=(0, 0, 2, 2))
        indices = index.query_points_in_radius(
            1.0, 1.0, 0.8
        )  # Query at (1,1) with radius 0.8
    ...
    def test_qpr_zero_radius(self):
        ...
        index_at_center = SpatialGridIndex(
            points_at_center, resolution=0.1, extent=(0, 0, 2, 2)
        )
        indices_at_center = index_at_center.query_points_in_radius(0.5, 0.5, 0.0)
        self.assertEqual(
            indices_at_center.size, 0
        )  # Still empty due to strict inequality
    ...
    def test_collinear_points(self):
        ...
        indices_c11 = index.query_points_in_cell(
            1, 1
        )  # Cell (1,1) should contain point 0
        assert_np_arrays_equal_after_sorting(self, indices_c11, [0])

        # Radius check
        indices_rad = index.query_points_in_radius(
            1.5, 1.5, 0.8
        )  # dist to (1,1) is ~0.707, dist to (2,2) is ~0.707
    ...
    def test_coincident_points(self):
        ...
        indices_rad_miss = index.query_points_in_radius(
            1.0, 1.0, 0.0
        )  # Strict inequality
    ...
    def test_points_on_cell_boundaries(self):
        ...
        indices_rad = index.query_points_in_radius(
            1.0, 1.0, 0.05
        )  # Small radius around (1.0,1.0)
        assert_np_arrays_equal_after_sorting(
            self, indices_rad, [1]
        )  # Should only find point [1]

        indices_rad_larger = index.query_points_in_radius(
            1.0, 1.0, 0.15
        )  # Radius to include all 3
