...
def test_simple_average_nodata():
    ...
    dtm = create_dtm_with_taichi_averaging(points, 1.0, dtm_extent_user=(0.0, 0.0, 2.0, 1.0), nodata_value=-1)
...
def test_run_simple_average_example():
    """Test the run_simple_average_example function to ensure it runs."""
    # Import the function from the module
    from algos.simple_average import run_simple_average_example
    
    # Run with minimal points and verbose off for speed
    try:
        dtm_array = run_simple_average_example(
            num_sample_points=5,
            extent_size=10.0,
            resolution=1.0,
            verbose=False
        )
        # Basic check that it returned a numpy array and it's not unexpectedly empty
        assert isinstance(dtm_array, np.ndarray)
        if 5 > 0: # If points were generated
             # Max possible dimension based on parameters for original grid logic
            max_dim_x = int(np.ceil((10.0 - 0.0 + 1e-6) / 1.0))
            max_dim_y = int(np.ceil((10.0 - 0.0 + 1e-6) / 1.0))
            # Actual dimensions can be smaller if random points don't span full extent_size
            assert dtm_array.shape[0] > 0 and dtm_array.shape[0] <= max_dim_y
            assert dtm_array.shape[1] > 0 and dtm_array.shape[1] <= max_dim_x
        else:
            assert dtm_array.shape == (1,0) # Expected for 0 points

    except Exception as e:
        pytest.fail(f"run_simple_average_example raised an exception: {e}")
...
def test_assign_points_outside_grid():
    ...
    points_inside = np.array([[0.25, 0.25, 10.0]], dtype=np.float32) # Should go to cell (0,0)
    ...
    dtm = create_dtm_with_taichi_averaging(
        all_points,
        resolution,
...
def test_invalid_grid_dimensions():
    ...
    dtm_invalid_extent = create_dtm_with_taichi_averaging(
        points, 
        dtm_resolution=1.0, 
