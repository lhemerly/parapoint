...
def idw_interpolation_kernel(
    ...
    for r, c in ti.ndrange(grid_height, grid_width):
        ...
        search_min_gx_float = (
            cell_center_x - search_radius - index_min_x
        ) / index_resolution
        search_max_gx_float = (
            cell_center_x + search_radius - index_min_x
        ) / index_resolution
        search_min_gy_float = (
            cell_center_y - search_radius - index_min_y
        ) / index_resolution
        search_max_gy_float = (
            cell_center_y + search_radius - index_min_y
        ) / index_resolution
        ...
        max_gix = ti.cast(
            ti.min(index_grid_dim_x - 1, ti.floor(search_max_gx_float)), ti.i32
        )
        min_giy = ti.cast(ti.max(0, ti.floor(search_min_gy_float)), ti.i32)
        max_giy = ti.cast(
            ti.min(index_grid_dim_y - 1, ti.floor(search_max_gy_float)), ti.i32
        )
        ...
        for cur_gix_loop in range(min_gix, max_gix + 1):
            ...
            for cur_giy_loop in range(min_giy, max_giy + 1):
                ...
                if not (
                    0 <= cur_gix < index_grid_dim_x and 0 <= cur_giy < index_grid_dim_y
                ):
                ...
                if num_points_in_cell > 0:
                    ...
                    for k_offset in range(num_points_in_cell):
                        original_point_idx = indexed_point_indices[
                            start_idx_in_flat_array + k_offset
                        ]
...
def idw(
    ...
    print(
        f"Starting DTM creation with Taichi (IDW): {ground_points_xyz.shape[0]} points, res {dtm_resolution}, radius {search_radius}, power {power}"
    )
    ...
    print(
        f"Initializing SpatialGridIndex for IDW with resolution: {dtm_resolution} over points extent X({idx_min_x:.2f}-{idx_max_x_adjusted:.2f}), Y({idx_min_y:.2f}-{idx_max_y_adjusted:.2f})"
    )
    ...
    if (
        spatial_index.num_points == 0 and points_x_np.size > 0
    ):  # check if points_x_np.size > 0 to ensure it's not just an empty input
        # This might happen if all points fall outside the provided extent or due to an issue in SpatialGridIndex init
        print(
            "Error: Spatial index created with zero points, but input points were provided. Check extent or SpatialGridIndex logs."
        )
        ...
        spatial_index.num_points == 0
    ):  # Handles case where input ground_points_xyz was empty and spatial_index is also empty
        print(
            "Warning: Spatial index is empty (likely due to no input points). DTM will be all nodata."
        )
        dtm_output = np.full((grid_height, grid_width), nodata_value, dtype=np.float32)
        return dtm_output
    ...
    idw_interpolation_kernel(
        ...
        search_radius,
        power,
        nodata_value,  # Kernel needs float for nodata
        # New parameters for spatial index:
