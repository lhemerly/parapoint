...
class SpatialGridIndex:
    def __init__(self, points_xy: np.ndarray, resolution: float, extent: tuple = None, min_points_per_cell_for_debug: int = 0):
        if not isinstance(points_xy, np.ndarray) or points_xy.ndim != 2 or points_xy.shape[1] != 2:
        ...
        if self.num_points == 0:
            ...
            self.grid_dim_x, self.grid_dim_y = 0, 0
            self.points_x_np = np.array([], dtype=np.float32)
            self.points_y_np = np.array([], dtype=np.float32)
            self.cell_point_counts = ti.field(dtype=ti.i32, shape=(0,0))
            self.cell_offsets = ti.field(dtype=ti.i32, shape=(0,0))
            self.indexed_point_indices = ti.field(dtype=ti.i32, shape=0)
            return
        ...
        if extent:
            ...
            if self.max_x == self.min_x: self.max_x += self.resolution 
            if self.max_y == self.min_y: self.max_y += self.resolution


        self.grid_dim_x = max(1, int(np.ceil((self.max_x - self.min_x) / self.resolution)))
        self.grid_dim_y = max(1, int(np.ceil((self.max_y - self.min_y) / self.resolution)))
        ...
        self.cell_point_counts = ti.field(dtype=ti.i32, shape=(self.grid_dim_x, self.grid_dim_y))
        self.cell_offsets = ti.field(dtype=ti.i32, shape=(self.grid_dim_x, self.grid_dim_y))
        cell_current_insertion_counts = ti.field(dtype=ti.i32, shape=(self.grid_dim_x, self.grid_dim_y))

        # 4. Call Kernel 1
        self.cell_point_counts.fill(0)
        assign_points_to_grid_and_count_kernel(
            points_x_ti, points_y_ti,
            self.min_x, self.min_y, self.resolution,
        ...
        if min_points_per_cell_for_debug > 0:
            ...
            if counts_np.max() > min_points_per_cell_for_debug:
                print(f"Warning: Max points in a single cell is {counts_np.max()}, "
        ...
        self.indexed_point_indices = ti.field(dtype=ti.i32, shape=max(0,total_indexed_points)) # Ensure non-negative shape
        ...
        if total_indexed_points > 0:
            ...
            populate_indexed_points_kernel(
                points_x_ti, points_y_ti, original_indices_ti,
                self.min_x, self.min_y, self.resolution,
                self.grid_dim_x, self.grid_dim_y,
                self.cell_offsets, cell_current_insertion_counts,
    ...
    def query_points_in_cell(self, cell_x_idx: int, cell_y_idx: int) -> np.ndarray:
        if not (0 <= cell_x_idx < self.grid_dim_x and 0 <= cell_y_idx < self.grid_dim_y):
            return np.array([], dtype=np.int32)
        if self.num_points == 0 or self.indexed_point_indices.shape[0] == 0 :
             return np.array([], dtype=np.int32)
    ...
    def query_points_in_radius(self, query_x: float, query_y: float, radius: float) -> np.ndarray:
...
if __name__ == '__main__':
    ...
    test_points_xy[:, 0] = test_points_xy[:, 0] * (test_max_val_x - test_min_val_x) + test_min_val_x
    test_points_xy[:, 1] = test_points_xy[:, 1] * (test_max_val_y - test_min_val_y) + test_min_val_y

    # Add specific points for targeted tests
    extra_points = np.array([
        [test_min_val_x, test_min_val_y],                  # Min corner
        [test_max_val_x - 1e-5, test_max_val_y - 1e-5],    # Near max corner (should be in last cell)
    ...
    effective_max_x = test_max_val_x + test_res * 0.1 # Ensure grid covers up to max_val_x
    ...
    spatial_index = SpatialGridIndex(test_points_xy, test_res, extent=(test_min_val_x, test_min_val_y, test_max_val_x, test_max_val_y), min_points_per_cell_for_debug=50)

    print(f"Grid definition: min=({spatial_index.min_x:.2f}, {spatial_index.min_y:.2f}), max=({spatial_index.max_x:.2f}, {spatial_index.max_y:.2f})")
    print(f"Grid dimensions: ({spatial_index.grid_dim_x}, {spatial_index.grid_dim_y}) cells")
    ...
    if spatial_index.indexed_point_indices.shape[0] > 0 or counts_sum > 0 : # Check if shape[0] is valid
        print(f"Length of final indexed_point_indices array: {spatial_index.indexed_point_indices.shape[0]}")
        assert counts_sum == spatial_index.indexed_point_indices.shape[0], "Sum of counts must match length of indexed array"
    ...
    if points_in_cell.size > 0:
        for pt_idx in points_in_cell:
            ...
            if not (actual_cx == test_cell_x and actual_cy == test_cell_y) :
                 print(f"  VERIFICATION ERROR: Point {pt_idx} ({px:.2f}, {py:.2f}) is in cell ({actual_cx},{actual_cy}), not ({test_cell_x},{test_cell_y})")
    ...
    if not np.array_equal(np.sort(points_in_radius), np.sort(np.array(manual_check_indices))):
    ...
    points_in_far_radius = spatial_index.query_points_in_radius(query_far_x, query_far_y, query_small_r)
    print(f"Found {points_in_far_radius.size} points: {points_in_far_radius} (expected 0)")
    ...
    print(f"Grid dimensions for empty index: ({empty_index.grid_dim_x}, {empty_index.grid_dim_y})")
    ...
    print(f"Grid for one point: dim=({one_point_index.grid_dim_x}, {one_point_index.grid_dim_y}), total_idx_pts={one_point_index.indexed_point_indices.shape[0]}")
